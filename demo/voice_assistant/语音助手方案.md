...
demo/
├── README.md                    # 总览：多项目说明
└── voice_assistant/             # 子项目：语音助手（包内按功能分子目录）
    ├── README.md
    ├── 语音助手方案.md
...
阶段一已实现：`llm/client.py` 封装 agents + `core/pipeline.py` 文本模式；`audio/asr.py`、`audio/tts.py` 为占位，阶段二接入真实 ASR/TTS。
...
- **内容**：
  - 在 `demo/voice_assistant/llm/` 下实现 `client.py`（LLM 客户端），封装对 `agents` 的调用（与现有 agent_system 一致，使用 Qwen3）。
  - 在 `demo/voice_assistant/core/` 下实现 `pipeline.py`：从标准输入读一行文本 → 调用 LLM → 打印回复。
...
- **内容**：
  - 集成 ModelScope FunASR（或项目允许的 ASR 方案），实现 `audio/asr.py`；从麦克风或音频文件得到文本。
  - 集成 ModelScope TTS（或项目允许的 TTS 方案），实现 `audio/tts.py`；将回复文本转为音频并播放。
  - 在 `core/pipeline.py` 中串联：录音 → ASR → LLM → TTS → 播放。
