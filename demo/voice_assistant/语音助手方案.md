...
**建议**：Demo 默认与 `agents/agent_system.py` 一致，通过环境变量 `USE_OLLAMA` / `SILICONFLOW_API_KEY` / `USE_MODELSCOPE` 切换，模型名统一用 Qwen3 系列。
...
| **对话模块** | 维护会话历史，调用 agents 的 LLM 生成回复。 | 复用 `agents/agent_system.py` 或其封装。 |
